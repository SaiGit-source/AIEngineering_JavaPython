...
→ AI calls OpenFoodFactsTool and/or NutriOpt OpenFoodFactsTool for nutrition facts and barcode lookup
...
- The OpenFoodFacts MCP server does not return the desired product.
...
### 17.5 Integration with OpenFoodFactsTool

The MCP client should ask OpenFoodFacts for:
...
> Build the MCP Client / DieticianAgent that connects to both OpenFoodFactsTool and this NutriOpt MCP Server, then revise this document with the full client architecture.
...
→ LLM calls OpenFoodFacts/OpenFoodFacts tools for product data
...
The original architecture expected the external OpenFoodFacts MCP server to provide nutrition and barcode lookup.
...
2. Gives direct fallback if OpenFoodFacts misses a product.
...
```text
1. Search product/food data through OpenFoodFacts and/or OpenFoodFactsTool.
...
## 30. React Frontend: NutriPlannerAI UI

The React frontend is the user-facing layer of the project. The current frontend name shown in the UI is:
...
The frontend is designed as a two-pane application:
...
│ AI Meal Planning + Diet Optimization                                         │
├───────────────────────────────────┬──────────────────────────────────────────┤
│ Left Pane: AI Chat                │ Right Pane: Meal Plan Dashboard          │
│                                   │                                          │
│ - User prompt                     │ - Today's optimized meal plan summary    │
│ - AI response                     │ - Breakfast, lunch, dinner cards         │
│ - Conversation ID                 │ - Macro progress bars                    │
│ - Loading/error messages          │ - Macro distribution chart               │
│                                   │ - Cost breakdown                         │
│                                   │ - Optimization status badges             │
└───────────────────────────────────┴──────────────────────────────────────────┘
```

The UI reference currently shows:

- a top-left title: **NutriPlannerAI**
- a left chat panel where the user asks for a meal plan
- a right visual dashboard with:
  - **Today’s Optimized Meal Plan**
  - calorie/protein/cost summary chips
  - **Breakfast**, **Lunch**, and **Dinner** cards
  - macro progress bars
  - a macro distribution donut chart
  - estimated daily cost
  - optimization status badges such as `RELAXED OPTIMIZATION` and `PRICE ESTIMATED`

The goal is that the user can read the full AI response on the left, while the right side converts the response into a clean decision-making dashboard.

---

## 31. React Frontend File Structure
...
## 32. React File Responsibilities
...
Main parent component. It arranges the page into header, left chat pane, and right dashboard pane.
...
Responsible for calling the Spring Boot MCP Client REST API.

Main call:

```text
GET /api/chat?question=...&conversationId=...
```

In local development, the React/Vite app runs on:

```text
http://localhost:5173
```

The Spring Boot MCP Client runs on:

```text
http://localhost:8085
```

Therefore, `vite.config.js` should proxy `/api` to port `8085`:
...
```text
components/layout/TwoPaneLayout.jsx
```

Creates the main left-right layout.

```text
components/layout/Pane.jsx
```

Reusable wrapper for a pane/card container.

---

### 32.4 Chat Components

```text
components/chat/ChatPane.jsx
```

The main left-side chat container. It displays:
...
Displays one message from either the user or NutriPlannerAI.

```text
components/chat/ConversationInfo.jsx
```

Displays the current conversation ID such as:
...
### 32.5 Dashboard Components

```text
components/dashboard/MealPlanDashboard.jsx
```

Main right-side dashboard container.

It combines:

- top meal summary card
...
```text
components/dashboard/DashboardSummaryCard.jsx
```

Top summary card, for example:
...
```text
components/dashboard/MealTimeline.jsx
```

Displays meals vertically as a daily timeline:
...
```text
components/dashboard/MealCard.jsx
```

Reusable meal card for breakfast, lunch, and dinner.
...
```text
components/dashboard/FoodItemRow.jsx
```

Displays one food item and quantity:
...
```text
components/dashboard/MacroProgressPanel.jsx
```

Container for macro progress bars.

```text
components/dashboard/MacroProgressBar.jsx
```

Displays one target progress row:
...
```text
components/dashboard/MacroDonutChart.jsx
```

Displays macro distribution:
...
```text
components/dashboard/CostBreakdownCard.jsx
```

Shows estimated cost:

```text
Estimated Daily Cost: $13.80
Breakfast: $2.45
Lunch: $5.20
Dinner: $6.15
```
...
```text
components/dashboard/OptimizationStatusCard.jsx
```

Shows whether the LP result was:
...
## 33. Badge Concept in the UI

A badge is a small visual label used to communicate status quickly.
...
In NutriPlannerAI, badges help the user understand the quality of the result without reading a long paragraph.
...
```text
[OPTIMAL]
The LP optimizer found a valid optimized result without relaxing constraints.

[RELAXED OPTIMIZATION]
The first LP attempt was infeasible, so the agent relaxed constraints and tried again.

[APPROXIMATE PLAN]
The LP optimizer could not find a feasible plan, so the agent created a best-effort meal plan manually.

[PRICE ESTIMATED]
Open Prices did not return exact price records, so estimated fallback prices were used.
```

---

## 34. React State and Data Flow

The first frontend version can use two kinds of data:

1. Full AI text response for the left chat pane.
2. Parsed meal plan object for the right dashboard.
...
```text
User types prompt
→ ChatPane receives prompt
→ useDieticianChat.sendMessage(prompt)
→ dieticianApi sends GET /api/chat
→ MCPClient returns AI text
→ Left pane displays full AI response
→ parseMealPlan extracts breakfast/lunch/dinner/totals/status
→ Right pane updates visual dashboard
```
...
└───────┬───────────────┘
        │
        ├──────────────► Left Pane: ChatMessage list
        │
        └──────────────► Right Pane: MealPlanDashboard
```

---

## 35. Big Picture End-to-End Architecture

The full system has four major layers:

```text
1. User Interface Layer
   React / NutriPlannerAI

2. Agent Layer
   Spring Boot MCPClient / DieticianAgent

3. Tool Server Layer
   Spring Boot NutriOpt Custom MCPServer

4. Data + Solver Layer
   OpenFoodFacts API, Open Prices API, MariaDB, OR-Tools
```
...
→ MCP tool calls
→ MCPServer tools
→ External APIs / MariaDB / OR-Tools
→ MCPClient receives results
→ AI formats meal plan
→ React displays chat + dashboard
```
...
┌──────────────────────────────────────────────────────────────────────────────┐
│ USER                                                                         │
│ "Create a vegetarian 2800 calorie meal plan with 120g protein"              │
└───────────────────────────────┬──────────────────────────────────────────────┘
                                │
                                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
...
│ - renders breakfast/lunch/dinner                                             │
│ - renders macro progress                                                     │
│ - renders cost and status badges                                             │
└───────────────────────────────┬──────────────────────────────────────────────┘
                                │ HTTP GET /api/chat
                                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ MCPCLIENT: DieticianAgent Spring Boot App                                    │
│                                                                              │
│ Controller: MCPClientController                                              │
│ - receives question and conversationId                                       │
│ - loads conversation history                                                 │
│ - builds agent prompt                                                        │
│ - calls Spring AI ChatClient                                                 │
│                                                                              │
│ ConversationHistoryService                                                   │
│ - stores user/assistant messages                                             │
│ - returns history block and summary                                          │
└───────────────────────────────┬──────────────────────────────────────────────┘
                                │ Spring AI tool calls over MCP
                                ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│ MCPSERVER: NutriOpt Custom MCP Server                                        │
│                                                                              │
│ Tools exposed to the agent:                                                  │
│                                                                              │
│ 1. OpenFoodFactsTool                                                         │
│    - search foods                                                            │
│    - lookup product by barcode/product code                                  │
│    - return calories/protein/carbs/fat/fiber                                 │
│                                                                              │
│ 2. OpenPricesTool                                                            │
│    - call Open Prices API by product_code/barcode                            │
│    - return crowdsourced price records when available                        │
│                                                                              │
│ 3. FoodStorageTool                                                           │
...
│                                                                              │
│ 4. LpOptimizerTool                                                           │
│    - receives MacroTargets and saved foodIds                                 │
│    - reads FoodItem rows from MariaDB                                        │
...
│                                                                              │
│ OpenFoodFacts API                                                            │
│ - product and nutrition facts                                                │
│                                                                              │
│ Open Prices API                                                              │
│ - price records by product code/barcode                                      │
│                                                                              │
│ MariaDB                                                                      │
│ - saved FoodItem rows                                                        │
│ - nutrition per 100g                                                         │
...
│ OR-Tools                                                                     │
│ - solves LP model                                                            │
│ - minimizes daily cost                                                       │
│ - satisfies calories, protein, carbs, fat, fiber, and budget constraints     │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

## 36. Detailed End-to-End Meal Planning Flow
...
3. MCPClient receives the request in MCPClientController.

4. MCPClient stores the user message using ConversationHistoryService.

5. MCPClient builds the DieticianAgent prompt.

6. Spring AI ChatClient receives the prompt.

7. ChatClient decides which MCP tools to call.

8. Agent calls OpenFoodFactsTool:
   - search affordable vegetarian foods
   - retrieve nutrition facts
   - retrieve barcode/product codes when available

9. Agent calls OpenPricesTool:
   - use barcode/product_code
   - retrieve price records if available
   - mark price as estimated if exact prices are missing

10. Agent calls FoodStorageTool.saveFoodItems:
    - save nutrition and price information into MariaDB
    - receive saved food IDs

11. Agent calls LpOptimizerTool.optimizeDietLp:
    - send MacroTargets
    - send saved foodIds

12. LpOptimizerTool reads FoodItem rows from MariaDB.

13. LpOptimizerTool calls LpSolverService.

14. LpSolverService uses OR-Tools GLOP solver:
    - decision variables = grams/ml per food
    - objective = minimize daily cost
    - constraints = calories, protein, carbs, fat, fiber, budget
...
16. If the first optimization is infeasible:
    - the agent relaxes constraints
    - adds more suitable foods if needed
    - calls optimizeDietLp again

17. Agent converts optimized daily quantities into meals:
    - Breakfast
    - Lunch
    - Dinner

18. MCPClient returns final AI response to React.

19. React displays:
    - full text response in the left chat pane
    - structured meal plan dashboard in the right pane
```

---

## 37. Frontend-to-Backend Runtime Requirements

For the full app to work locally, the services should run in this order:

```text
1. MariaDB
2. MCPServer / NutriOpt Custom MCP Server
3. MCPClient / DieticianAgent Spring Boot app
4. React frontend / NutriPlannerAI
```

However, if MCPClient is configured to launch the MCPServer through its MCP stdio server configuration, then the manual run order becomes:

```text
1. MariaDB
2. MCPClient / DieticianAgent Spring Boot app
3. React frontend / NutriPlannerAI
```
...
then the MCPClient backend is probably not running or the Vite proxy port is wrong.
...
then React reached the MCPClient, but the MCPClient failed internally. The MCPClient terminal stack trace should be checked.

---

## 38. React Dashboard Data Strategy

The current backend returns a text response.

Therefore, the first frontend version can do this:
...
Example parsed dashboard object:
...
```json
{
  "summary": {
    ...
    "protein": 123,
    "cost": 13.80,
    "badges": ["RELAXED OPTIMIZATION", "PRICE ESTIMATED"]
  },
  "meals": {
    "breakfast": {
      "title": "Breakfast",
  ...
  "totals": {
    "calories": { "actual": 2810, "target": 2800, "unit": "kcal" },
    "protein": { "actual": 123, "target": 120, "unit": "g" },
    "carbs": { "actual": 365, "target": 400, "unit": "g" },
    "fat": { "actual": 82, "target": 90, "unit": "g" },
    "fiber": { "actual": 31, "target": 25, "unit": "g" }
  }
}
```
...
```text
MCPClient should return structured JSON instead of only plain text.
```

Better future response shape:

```json
{
  "conversationId": "veg-5298",
  "answer": "Full natural language AI response...",
  "mealPlan": {
    "breakfast": [],
    "lunch": [],
    "dinner": [],
    "totals": {},
    "badges": [],
    "optimizationStatus": "RELAXED_OPTIMIZATION"
  }
}
```

This will make the React dashboard more reliable and avoid parsing fragile text.

---

## 39. Updated Final Big Picture

NutriPlannerAI is the frontend name of the application.

DieticianAgent is the backend AI agent role.

NutriOpt Custom MCP Server is the tool server.
...
DieticianAgent
= Spring Boot MCPClient
= AI reasoning, prompt building, conversation memory, tool selection

NutriOpt Custom MCP Server
= Spring Boot MCPServer
= OpenFoodFacts, Open Prices, MariaDB storage, OR-Tools LP optimizer
```
...
→ NutriPlannerAI sends the prompt to DieticianAgent
→ DieticianAgent uses MCP tools from NutriOpt MCPServer
→ OpenFoodFacts provides nutrition
→ Open Prices provides price data when available
→ MariaDB stores food records and food IDs
→ OR-Tools solves the diet LP
→ DieticianAgent converts quantities into breakfast/lunch/dinner
→ NutriPlannerAI displays the chat response and visual dashboard
```
